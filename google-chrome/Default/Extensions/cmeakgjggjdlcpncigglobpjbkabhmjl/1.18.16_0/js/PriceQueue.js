  ...
  _setSteamPrice(response, url, providerName) {
    ...
    if (STEAM_BY_SIH_SUPPORTED_APPS.includes(+appid)) {
      if (
        ...
        if (appid == 753 && currencyId == 5) {
          chrome.runtime.sendMessage(SIHID, {type: 'BACKGROUND_SET_CARD_STEAM_PRICE', data: {
              steamid: g_steamID,
...
const MarketplaceSource = {
  ...
  _getMarketHashNamesInChunks(appid, market, chunkSize = 5000) {
    let names = [];
    const $itemsBlock = !window.location.href.match(/tradeoffers/) ? $J('body') : $J('.sih_tradeoffer.active_single_trade').length > 0 ? $J('.sih_tradeoffer.active_single_trade') : $J('.sih_tradeoffer');
  ...
  _createPricesHandler: (market, appid) => async ({success, items}, hashNames) => {
    if (success) {
      ...
      const result = hashNames.reduce((acc, val) =>  ({...acc, [val]: {[market]: {price: items[val] ? items[val][market].price : null}}}), {});
